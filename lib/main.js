...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            core.exportVariable('ENV_VARIABLE', 'test1');
            core.setOutput('ENV_VARIABLE', 'test1');
            console.log(core.getInput('ENV_VARIABLE', { required: false }));
        }
        catch (error) {
            if (!!error.errors && error.errors[0].message == 'No commits between ' + context.repo.owner + ':' + base + ' and ' + owner + ':' + head) {
                core.exportVariable('ENV_VARIABLE', 'test2');
                core.setOutput('ENV_VARIABLE', 'test2');
                console.log(core.getInput('ENV_VARIABLE', { required: false }));
